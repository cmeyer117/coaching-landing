...
## Formula (research-confirmed, matches ISSN/Helms/Barbell Medicine consensus for a simple beginner calculator)
...
6. **Carbs** — remaining calories (`calories - protein×4 - fat×9`) ÷ 4 kcal/g.

Implemented as one pure function, `calculateMacros(inputs) → { calories, proteinG, fatG, carbG }`, unit-agnostic internally (converts lb→kg, ft/in→cm at the boundary).
...
6. Insert success → results panel reveals inline (calories + 3 macros in grams), plus a CTA: "Ready for real coaching?" linking to `index.html#apply`.
